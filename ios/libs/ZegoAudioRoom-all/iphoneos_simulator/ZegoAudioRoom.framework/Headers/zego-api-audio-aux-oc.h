 ...
 * 2.用户根据实际的 PCM 音频填写采样率及声道数。
 * 3.为确保混音效果，请不要在此 API 中执行耗时操作。
 @note 1. 发送媒体次要信息前需要调用 [ZegoMediaSideInfo -setMediaSideFlags:onlyAudioPublish:mediaInfoType:seiSendType:channelIndex:] 开启媒体次要信息传输功能。
 ...
 * 2.此接口带媒体次要信息时必须要带有混音音频数据(不能只有媒体次要信息)才能发送媒体次要信息，如果不带混音音频数据则会丢掉媒体次要信息。
 ...
 @see 相关接口请查看 ZegoAudioAux -enableAux:
 */
...
/**
 ...
 @note 若不再需要混音，调用 setDelegate:nil 去除回调监听。
 @see 相关接口请查看  ZegoAudioAux -enableAux:
 */
 ...
 * 2.当开启混音后，SDK 会在 -onAuxCallback:dataLen:sampleRate:channelCount:sideInfo:sideInfoLen:packet: 中获取用于混音的音频数据，即需要开发者在此接口中塞音频数据给 SDK.
 ...
 @see 相关接口请查看 -onAuxCallback:dataLen:sampleRate:channelCount:sideInfo:sideInfoLen:packet:
 */
