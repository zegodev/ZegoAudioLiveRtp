...
/**
 ...
 @note 注意：1. 必须在调用初始化 SDK 接口之后、推流接口之前设置。
 ...
 * 2.如果只有音频直播，必须将 onlyAudioPublish 置为 true，此时将会由音频来驱动次要信息的传输，同时忽略视频流传输。
 ...
 @param channelIndex    推流通道 index，请参考 ZegoAPIPublishChannelIndex
 */
 ...
 * 4.sendMediaSideInfo 的调用频率不能超过帧率，假设推流采用默认帧率15 fps，即调用频率不能超过 1000/15=66.7 ms/次。
 ...
 @param channelIndex 推流通道 index，请参考 ZegoAPIPublishChannelIndex
 @see 相关接口请查看 ZegoMediaSideInfo -setMediaSideFlags:onlyAudioPublish:mediaInfoType:seiSendType:channelIndex: 或者 ZegoMediaSideInfo -setMediaSideFlags:onlyAudioPublish:channelIndex:
 */
...
/**
 ...
 @note 注意：1. 在调用初始化 SDK 接口之后、拉流接口之前设置。
 ...
 * 4.当不需要接收数据时，调用 setMediaSideDelegate:nil 去除回调监听，避免内存泄漏。
