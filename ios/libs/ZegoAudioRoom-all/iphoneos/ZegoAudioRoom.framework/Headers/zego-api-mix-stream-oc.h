 ...
 * 调用 ZegoStreamMixer -setDelegate: 设置了回调监听，并调用 ZegoStreamMixer -mixStream:seq: 设置混流配置后，SDK 通过此 API 通知调用方混流配置信息。
 ...
 @param info 混流播放信息
 @note 常见错误码及其含义请参考 ZegoError 中 kMixStream 开头的错误码定义。
 ...
 * 调用 ZegoStreamMixer -setMixStreamExDelegate: 设置了回调监听，并调用 ZegoStreamMixer -mixStreamEx:mixStreamID: 设置混流配置后，SDK 通过此 API 通知调用方混流配置结果。
 ...
 @param info 混流播放信息
 @note 1. 如果输出流列表中输出流使用 URL 作为输出，则此回调中的 info 参数中不会包含有效的 rtmp、hls、flv 格式的混流信息，即使用该参数中的 url 不能拉取混流。
...
/**
 ...
 @note Deprecated 此方法不建议使用，建议使用 ZegoStreamMixer -setMixStreamExDelegate: 和 ZegoStreamMixer -mixStreamEx:mixStreamID:
 
 * 若不再需要接收混流配置更新信息，调用 setDelegate:nil 去除回调监听。
 @see 相关接口请查看 ZegoStreamMixer -mixStream:seq:
 */
 ...
 * 若不再需要接收混流中各单流的音量信息，调用 setSoundLevelInMixedStreamDelegate:nil 去除回调监听。
 */
...
/**
 ...
 @note 若不再需要接收混流配置更新信息，调用 setMixStreamExDelegate:nil 去除回调监听。
 @param delegate 实现了 ZegoMixStreamExDelegate 代理的方法，用于混流时接收混流请求结果及配置更新信息。
 @see 相关接口请查看 ZegoStreamMixer -mixStreamEx:mixStreamID:
 */
